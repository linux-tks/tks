...
pub fn register_org_freedesktop_secret_prompt<T>(cr: &mut crossroads::Crossroads) -> crossroads::IfaceToken<T>
where T: OrgFreedesktopSecretPrompt + Send + 'static
{
    cr.register("org.freedesktop.Secret.Prompt", |b| {
        b.signal::<(bool,arg::Variant<Box<dyn arg::RefArg + 'static>>,), _>("Completed", ("dismissed","result",));
        b.method("Prompt", ("window_id",), (), |_, t: &mut T, (window_id,)| {
            t.prompt(window_id,)
        });
        b.method("Dismiss", (), (), |_, t: &mut T, ()| {
            t.dismiss()
        });
    })
}
