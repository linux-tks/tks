...
pub trait OrgFreedesktopSecretCollection {
    ...
    fn search_items(&mut self, attributes: ::std::collections::HashMap<String, String>) -> Result<Vec<dbus::Path<'static>>, dbus::MethodErr>;
    fn create_item(&mut self, properties: arg::PropMap, secret: (dbus::Path<'static>, Vec<u8>, Vec<u8>, String), replace: bool) -> Result<(dbus::Path<'static>, dbus::Path<'static>), dbus::MethodErr>;
...
pub fn register_org_freedesktop_secret_collection<T>(cr: &mut crossroads::Crossroads) -> crossroads::IfaceToken<T>
where T: OrgFreedesktopSecretCollection + Send + 'static
{
    cr.register("org.freedesktop.Secret.Collection", |b| {
        ...
        b.method("SearchItems", ("attributes",), ("results",), |_, t: &mut T, (attributes,)| {
            t.search_items(attributes,)
                .map(|x| (x,))
        })
            .annotate("org.qtproject.QtDBus.QtTypeName.In0", "StrStrMap");
        b.method("CreateItem", ("properties","secret","replace",), ("item","prompt",), |_, t: &mut T, (properties,secret,replace,)| {
